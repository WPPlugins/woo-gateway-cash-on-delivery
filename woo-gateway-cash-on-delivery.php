...
function init_cash_on_delivery_gateway_class() {	
	...
	function add_cash_on_delivery_gateway_class( $methods ) {
		...
		if ( !is_admin() ) {
			...
			$chosen_methods_meta = woo_check_the_name_ofthe_delivery_cash($chosen_methods[0]);	
		...
		if ($cash_on_delivery_settings[enabled] == "yes") {
			if( is_array($cash_on_delivery_settings[enable_for_methods]) && in_array($chosen_methods_meta, $cash_on_delivery_settings[enable_for_methods]) ) {			
				$methods[] = 'Woo_Gateway_Cash_On_Delivery';	
				return $methods;
			} else {
				return $methods;
